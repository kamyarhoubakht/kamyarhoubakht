...
if id -u "$sudo_user" &>/dev/null; then
  ...
  useradd -m -s /bin/bash "$sudo_user" || { log_error "Failed to create user '$sudo_user'."; exit 1; }
...
if ! step_done "docker"; then
  ...
  echo "deb [arch=$(dpkg --print-architecture) signed-by=/etc/apt/keyrings/docker.asc] https://download.docker.com/linux/${OS_ID} $(lsb_release -cs) stable" \
...
# Add user to docker group (cheap, always safe to re-run)
