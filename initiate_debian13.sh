...
EOF
    ...
    # Docker's native nftables firewall backend requires Docker Engine 29+.
    DOCKER_VERSION="$(docker version --format '{{.Server.Version}}' 2>/dev/null || true)"
    if [[ -z "$DOCKER_VERSION" ]]; then
        log_error "Could not determine the installed Docker Engine version."
        exit 1
    fi

    if ! dpkg --compare-versions "$DOCKER_VERSION" ge "29.0.0"; then
        log_error "Docker Engine $DOCKER_VERSION is too old for the nftables firewall backend."
        log_error "Docker Engine 29.0.0 or newer is required."
        exit 1
    fi

    log_success "Docker Engine $DOCKER_VERSION supports the native nftables firewall backend."

    # Docker's nftables backend does not enable IP forwarding itself.
    log_step "Enabling IP forwarding for Docker nftables networking"
    cat > /etc/sysctl.d/99-docker-nftables.conf <<'EOF'
...
EOF
    ...
    # Configure Docker to use nftables for its container firewall/NAT rules.
    log_step "Configuring Docker to use the native nftables firewall backend"
    install -d -m 0755 /etc/docker

    if [[ -f /etc/docker/daemon.json && -s /etc/docker/daemon.json ]]; then
        if grep -q '"firewall-backend"[[:space:]]*:[[:space:]]*"nftables"' /etc/docker/daemon.json; then
            log_success "Docker nftables firewall backend is already configured."
        else
            BACKUP_FILE="/etc/docker/daemon.json.pre-nftables-$(date +%Y%m%d_%H%M%S)"
            cp -a /etc/docker/daemon.json "$BACKUP_FILE"
            log_error "Existing /etc/docker/daemon.json does not configure the nftables backend."
            log_error "A backup was created at: $BACKUP_FILE"
            log_error "The script will not overwrite an existing Docker configuration."
            log_error "Add \"firewall-backend\": \"nftables\" to that file and rerun the script."
            exit 1
        fi
    else
...
EOF
        ...
        log_success "Docker nftables firewall backend configured."
    fi

    if ! dockerd --validate --config-file=/etc/docker/daemon.json; then
        log_error "Docker daemon configuration is invalid."
        exit 1
    fi
